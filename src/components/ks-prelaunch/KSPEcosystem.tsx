        ...
        <div className="grid sm:grid-cols-3 gap-4 sm:gap-6 mb-14">
          {products.map((product, i) => (
            <ScrollReveal key={i} delay={0.3 + i * 0.1}>
              <div className="group">
                <div className="relative aspect-square rounded-2xl overflow-hidden ring-1 ring-white/10 bg-transparent hover:ring-primary/30 transition-all duration-500 mb-5">
                  {/* Subtle radial glow behind product */}
                  <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse at center 60%, hsl(var(--primary) / 0.15), transparent 70%)' }} />
                  {product.image ? (
                    <img
                      src={product.image}
                      alt={product.name}
